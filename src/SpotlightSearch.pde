...
void setup()
{
    ...
    for (int i = 0; i < 96; i++){
        ...
        image(iconToBeAdded.iconImage, iconToBeAdded.xCoord, iconToBeAdded.yCoord, 50, 50);
        
        // Place text on screen
        fill(0);
        text(iconToBeAdded.iconName, xText, yText);
        
        // Draw search Bar
        fill(255);
        rect(350, 750, 500, 45);
        
        icons.add(iconToBeAdded);
    }
}
...
ArrayList<String> iconsThatMatch = new ArrayList<>();
...
{  
    ...
    {
        if (currentTypedString.length() == 0)
        {
            println("There is nothing to delete.");
            for (Icon icon : icons)
            {
                icon.redrawIcon(State.WITHOUT_BLUR);
            }
        }
        ...
        if (currentTypedString.length() > 0)
        {
            ...
            // Get the last position of the string (last char typed)
            textSize(30);
            float lastCharWidth = textWidth(currentTypedString.substring(currentTypedString.length() - 1));
            
            // Cover it with a rectangle
            rect(370 + textWidth(currentTypedString) - lastCharWidth, 755, lastCharWidth, 38);
            
            // Remove the char from the string
            char charDeleted =  currentTypedString.charAt(currentTypedString.length() - 1);
            currentTypedString = currentTypedString.substring(0, currentTypedString.length() - 1);
            
            // Redo type-based filter search with shortned string
            iconsThatMatch.clear();
            for (Icon icon: icons)
            {
                icon.isFlagged = false;
                icon.redrawIcon(State.WITH_BLUR);
                if (icon.fullName.toLowerCase().contains(currentTypedString) || 
                    icon.fullName.toUpperCase().contains(currentTypedString.toUpperCase()))
                {
                  iconsThatMatch.add(icon.fullName);
                  icon.redrawIcon(State.WITHOUT_BLUR);
                  icon.isFlagged = true;
                }
            }
          
          println("List of apps found with search after deletion:" + "\n");
          for (String appName : iconsThatMatch)
          {
              println(appName);
          }
          println("Apps found with search: " + iconsThatMatch.size());
            
          println("\n" + charDeleted + " has been deleted.");
        }
    ...
    else
    {
        if (key != ENTER && key != RETURN)
        {
            ...
            iconsThatMatch.clear();
            // Iterate through icon list, add any icons that match the search
            for (Icon icon: icons)
            {
              icon.isFlagged = false;
              icon.redrawIcon(State.WITH_BLUR);
            ...
            println("List of apps found with search:" + "\n");
            for (String appName : iconsThatMatch)
            {
                println(appName);
            }
            ...
            fill(0);
            textSize(30);
            text(currentTypedString, 370, 784);
            
            println(currentTypedString);
        }   
    }
}
...
void mousePressed()
{
    // Clear the current typed string for the next input
    currentTypedString = "";
